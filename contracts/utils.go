...
func CreateTransactionSign(chainConfig *params.ChainConfig, pool *core.TxPool, manager *accounts.Manager, block *types.Block, chainDb ethdb.Database) error {
	...
	if chainConfig.DPoS != nil {
		...
		checkNumber := blockNumber % chainConfig.DPoS.Epoch
		...
		if !exist && checkNumber > 0 && common.EpocBlockSecret <= checkNumber && common.EpocBlockOpening > checkNumber {
			...
			tx, err := BuildTxSecretRandomize(nonce+1, common.HexToAddress(common.RandomizeSMC), chainConfig.DPoS.Epoch, randomizeKeyValue)
			if err != nil {
...
func BuildValidatorFromM2(listM2 []int64) []byte {
	...
	for _, numberM2 := range listM2 {
		...
		m2Byte := common.LeftPadBytes([]byte(fmt.Sprintf("%d", numberM2)), DPoS.M2ByteLength)
...
func DecodeValidatorsHexData(validatorsStr string) ([]int64, error) {
	...
	return DPoS.ExtractValidatorsFromBytes(validatorsByte), nil
}
...
func GetRewardForCheckpoint(c *DPoS.DPoS, chain consensus.ChainReader, header *types.Header, rCheckpoint uint64, totalSigner *uint64) (map[common.Address]*rewardLog, error) {
	...
	masternodes := DPoS.GetMasternodesFromCheckpointHeader(header)
