...
func main() {
	...
	trc21IssuerAddr, trc21Issuer, err := sdxx.DeployTRC21Issuer(auth, client, simulation.MinTRC21Apply)
	if err != nil {
	...
	sdxxListtingAddr, sdxxListing, err := sdxx.DeployTOMOXListing(auth, client)
	if err != nil {
	...
	applyTomoXListing(sdxxListing, tokenList, currentNonce)
...
func initTRC21(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		tokenAddr, _, err := sdxx.DeployTRC21(auth, client, simulation.Owners, simulation.Required, tokenName, tokenName, d, tokenCap, simulation.TRC21TokenFee, depositFee, withdrawFee)
		if err != nil {
...
func applyIssuer(trc21Issuer *sdxx.TRC21Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
...
func applyTomoXListing(sdxxListing *sdxx.TOMOXListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		sdxxListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		sdxxListing.TransactOpts.Value = simulation.TomoXListingFee
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			trc21Contract, _ := sdxx.NewTRC21(auth, token["address"].(common.Address), client)
