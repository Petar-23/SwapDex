...
import (
	...
	simulation "github.com/69th-byte/SmartDex-Chain/contracts/sdxx/testnet"
...
func main() {
	...
	src21IssuerAddr, src21Issuer, err := sdxx.DeploySRC21Issuer(auth, client, simulation.MinSRC21Apply)
	if err != nil {
		log.Fatal("DeploySRC21Issuer", err)
	}
	src21Issuer.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> sdx21 issuer address", src21IssuerAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : SDX Issuer")
	time.Sleep(2 * time.Second)

	//init SDXX Listing in
	auth.Nonce = big.NewInt(int64(nonce + 1))
	sdxxListtingAddr, sdxxListing, err := sdxx.DeploySDXXListing(auth, client)
	if err != nil {
		log.Fatal("DeploySDXXListing", err)
	}
	sdxxListing.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> sdxx listing address", sdxxListtingAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : sdxx listing ")
	time.Sleep(2 * time.Second)

	// init Relayer Registration
	auth.Nonce = big.NewInt(int64(nonce + 2))
	relayerRegistrationAddr, relayerRegistration, err := sdxx.DeployRelayerRegistration(auth, client, sdxxListtingAddr, simulation.MaxRelayers, simulation.MaxTokenList, simulation.MinDeposit)
	if err != nil {
	...
	lendingRelayerRegistrationAddr, lendingRelayerRegistration, err := sdxx.DeployLendingRelayerRegistration(auth, client, relayerRegistrationAddr, sdxxListtingAddr)
	if err != nil {
	...
	tokenList := initSRC21(auth, client, currentNonce, simulation.TokenNameList)

	currentNonce = currentNonce + uint64(len(simulation.TokenNameList)) // init smartcontract

	applyIssuer(src21Issuer, tokenList, currentNonce)

	currentNonce = currentNonce + uint64(len(simulation.TokenNameList))
	applySdxXListing(sdxxListing, tokenList, currentNonce)
	...
	relayerRegistration, err = sdxx.NewRelayerRegistration(ownerRelayer, relayerRegistrationAddr, client)
	if err != nil {
	...
	*/
	...
	fmt.Println("wait 2s to apply token to list sdxx")
	time.Sleep(2 * time.Second)

	// Lending apply
	nonce = nonce + 1
	lendingRelayerRegistration, err = sdxx.NewLendingRelayerRegistration(ownerRelayer, lendingRelayerRegistrationAddr, client)
	if err != nil {
		log.Fatal("NewRelayerRegistration", err)
	}
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
...
func initSRC21(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		if tokenName == "USDT" {
			d = 6
			tokenCap = new(big.Int).Div(simulation.SRC21TokenCap, big.NewInt(1000000000000))
			withdrawFee = big.NewInt(970000)
		}
		if tokenName == "BTC" {
			d = 8
			tokenCap = new(big.Int).Div(simulation.SRC21TokenCap, big.NewInt(10000000000))
			withdrawFee = big.NewInt(40000)
		}
		if tokenName == "ETH" {
			withdrawFee = big.NewInt(3000000000000000)
		}
		tokenAddr, _, err := sdxx.DeploySRC21(auth, client, simulation.Owners, simulation.Required, tokenName, tokenName, d, tokenCap, simulation.SRC21TokenFee, depositFee, withdrawFee)
		if err != nil {
			log.Fatal("DeploySRC21 ", tokenName, err)
		}
...
func applyIssuer(src21Issuer *sdxx.SRC21Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		src21Issuer.TransactOpts.Nonce = big.NewInt(int64(nonce))
		src21Issuer.TransactOpts.Value = simulation.MinSRC21Apply
		_, err := src21Issuer.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("src21Issuer Apply  ", token["name"].(string), err)
		}
...
func applySdxXListing(sdxxListing *sdxx.SDXXListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		sdxxListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		sdxxListing.TransactOpts.Value = simulation.SdxXListingFee
		_, err := sdxxListing.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("sdxxListing Apply ", token["name"].(string), err)
		}
		fmt.Println("applySdxXListing ", token["name"].(string))
		nonce = nonce + 1
	}
	time.Sleep(5 * time.Second)
}
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			src21Contract, _ := sdxx.NewSRC21(auth, token["address"].(common.Address), client)
			src21Contract.TransactOpts.Nonce = big.NewInt(int64(nonce))
			...
			_, err := src21Contract.Transfer(address, amount)
			if err == nil {
