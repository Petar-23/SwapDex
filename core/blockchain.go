...
func NewBlockChainEx(db ethdb.Database, sdxxDb ethdb.TomoxDatabase, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	blockchain, err := NewBlockChain(db, cacheConfig, chainConfig, engine, vmConfig)
	if err != nil {
		return nil, err
	}
	if blockchain != nil {
		blockchain.addTomoxDb(sdxxDb)
	}
...
func (bc *BlockChain) addTomoxDb(sdxxDb ethdb.TomoxDatabase) {
	bc.sdxxDb = sdxxDb
}
...
func (bc *BlockChain) OrderStateAt(block *types.Block) (*tradingstate.TradingStateDB, error) {
	engine, ok := bc.Engine().(*posv.Posv)
	if ok {
		sdxXService := engine.GetTomoXService()
		if bc.Config().IsTIPTomoX(block.Number()) && bc.chainConfig.Posv != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch && sdxXService != nil {
