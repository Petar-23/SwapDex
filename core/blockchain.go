...
func (bc *BlockChain) insert(block *types.Block) {
	...
	if bc.chainConfig.DPoS != nil && !bc.chainConfig.IsTIPSigning(block.Number()) {
		engine := bc.Engine().(*DPoS.DPoS)
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		if status == CanonStatTy && bc.chainConfig.DPoS != nil {
			// epoch block
			if (chain[i].NumberU64() % bc.chainConfig.DPoS.Epoch) == 0 {
				CheckpointCh <- 1
			}
			// prepare set of masternodes for the next epoch
			if (chain[i].NumberU64() % bc.chainConfig.DPoS.Epoch) == (bc.chainConfig.DPoS.Epoch - bc.chainConfig.DPoS.Gap) {
				err := bc.UpdateM1()
				if err != nil {
...
func (bc *BlockChain) insertBlock(block *types.Block) ([]interface{}, []*types.Log, error) {
	...
	if status == CanonStatTy && bc.chainConfig.DPoS != nil {
		// epoch block
		if (block.NumberU64() % bc.chainConfig.DPoS.Epoch) == 0 {
			CheckpointCh <- 1
		}
		// prepare set of masternodes for the next epoch
		if (block.NumberU64() % bc.chainConfig.DPoS.Epoch) == (bc.chainConfig.DPoS.Epoch - bc.chainConfig.DPoS.Gap) {
			err := bc.UpdateM1()
			if err != nil {
...
func (bc *BlockChain) UpdateM1() error {
	...
	for _, candidate := range candidates {
		...
		if candidate.String() != "0x0000000000000000000000000000000000000000" {
			ms = append(ms, DPoS.Masternode{Address: candidate, Stake: v})
		}
	}
	if len(ms) == 0 {
