...
func (self *worker) wait() {
	for {
		...
		for result := range self.recv {
			...
			block := result.Block
			if self.config.DPoS != nil && block.NumberU64() >= self.config.DPoS.Epoch && len(block.Validator()) == 0 {
			...
			if work.config.DPoS != nil {
				// epoch block
				if (block.NumberU64() % work.config.DPoS.Epoch) == 0 {
					core.CheckpointCh <- 1
				}
				// prepare set of masternodes for the next epoch
				if (block.NumberU64() % work.config.DPoS.Epoch) == (work.config.DPoS.Epoch - work.config.DPoS.Gap) {
...
func (self *worker) commitNewWork() {
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		if self.config.DPoS != nil {
			...
			if !ok {
				...
				h := DPoS.Hop(len, preIndex, curIndex)
				gap := waitPeriod * int64(h)
				// Check nearest checkpoint block in hop range.
				nearest := self.config.DPoS.Epoch - (parent.Header().Number.Uint64() % self.config.DPoS.Epoch)
				if uint64(h) >= nearest {
	...
	if self.config.DPoS != nil && header.Number.Uint64()%self.config.DPoS.Epoch != 0 {
...
func (env *Work) commitTransactions(mux *event.TypeMux, txs *types.TransactionsByPriceAndNonce, specialTxs types.Transactions, bc *core.BlockChain, coinbase common.Address) {
	...
	for _, tx := range specialTxs {
		...
		if tx.To().Hex() == common.BlockSigners {
			...
			if blkNumber >= env.header.Number.Uint64() || blkNumber <= env.header.Number.Uint64()-env.config.DPoS.Epoch*2 {
