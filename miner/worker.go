...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	if self.config.Posv != nil {
		...
		lending := self.eth.GetSdxXLending()
		...
		if self.config.Posv != nil && self.chain.Config().IsTIPSdxX(header.Number) {
			...
			if sdxX != nil && header.Number.Uint64() > self.config.Posv.Epoch {
				if header.Number.Uint64()%self.config.Posv.Epoch == 0 {
					err := sdxX.UpdateMediumPriceBeforeEpoch(header.Number.Uint64()/self.config.Posv.Epoch, work.tradingState, work.state)
					if err != nil {
				...
				//https://github.com/69th-byte/SmartDex-Chain-v1/pull/416
				if header.Number.Uint64()%self.config.Posv.Epoch != 0 {
					...
					tradingTxMatches, tradingMatchingResults = sdxX.ProcessOrderPending(header, self.coinbase, self.chain, tradingOrderPending, work.state, work.tradingState)
					...
					lendingInput, lendingMatchingResults = sdxXLending.ProcessOrderPending(header, self.coinbase, self.chain, lendingOrderPending, work.state, work.lendingState, work.tradingState)
					...
					if header.Number.Uint64()%self.config.Posv.Epoch == common.LiquidateLendingTradeBlock {
						updatedTrades, liquidatedTrades, autoRepayTrades, autoTopUpTrades, autoRecallTrades, err = sdxXLending.ProcessLiquidationData(header, self.chain, work.state, work.tradingState, work.lendingState)
						if err != nil {
				...
				if len(lendingInput) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					lendingTx := types.NewTransaction(nonce, common.HexToAddress(common.SdxXLendingAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), lendingDataBytes)
				...
				if len(updatedTrades) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					finalizedTx := types.NewTransaction(nonce, common.HexToAddress(common.SdxXLendingFinalizedTradeAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), finalizedTradeData)
