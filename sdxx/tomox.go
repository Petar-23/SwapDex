...
func (sdxx *TomoX) Protocols() []p2p.Protocol {
	return []p2p.Protocol{}
}

func (sdxx *TomoX) Start(server *p2p.Server) error {
	return nil
}

func (sdxx *TomoX) SaveData() {
}
func (sdxx *TomoX) Stop() error {
	return nil
}
...
func (sdxx *TomoX) Overflow() bool {
	val, _ := sdxx.settings.Load(overflowIdx)
	return val.(bool)
}

func (sdxx *TomoX) IsSDKNode() bool {
	return sdxx.sdkNode
}

func (sdxx *TomoX) GetLevelDB() sdxxDAO.sdxXDAO {
	return sdxx.db
}

func (sdxx *TomoX) GetMongoDB() sdxxDAO.sdxXDAO {
	return sdxx.mongodb
}

// APIs returns the RPC descriptors the TomoX implementation offers
func (sdxx *TomoX) APIs() []rpc.API {
	return []rpc.API{
		{
			Namespace: ProtocolName,
			Version:   ProtocolVersionStr,
			Service:   NewPublicTomoXAPI(sdxx),
			Public:    true,
		},
	}
}
...
func (sdxx *TomoX) Version() uint64 {
	return ProtocolVersion
}

func (sdxx *TomoX) ProcessOrderPending(header *types.Header, coinbase common.Address, chain consensus.ChainContext, pending map[common.Address]types.OrderTransactions, statedb *state.StateDB, sdxXstatedb *tradingstate.TradingStateDB) ([]tradingstate.TxDataMatch, map[common.Hash]tradingstate.MatchingResult) {
...
func (sdxx *TomoX) GetAveragePriceLastEpoch(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, baseToken common.Address, quoteToken common.Address) (*big.Int, error) {
...
func (sdxx *TomoX) ConvertTOMOToToken(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, token common.Address, quantity *big.Int) (*big.Int, *big.Int, error) {
...
func (sdxx *TomoX) SyncDataToSDKNode(takerOrderInTx *tradingstate.OrderItem, txHash common.Hash, txMatchTime time.Time, statedb *state.StateDB, trades []map[string]string, rejectedOrders []*tradingstate.OrderItem, dirtyOrderCount *uint64) error {
	var (
...
func (sdxx *TomoX) GetTradingState(block *types.Block, author common.Address) (*tradingstate.TradingStateDB, error) {
...
func (sdxx *TomoX) GetStateCache() tradingstate.Database {
	return sdxx.StateCache
}
func (sdxx *TomoX) HasTradingState(block *types.Block, author common.Address) bool {
	root, err := sdxx.GetTradingStateRoot(block, author)
	if err != nil {
		return false
	}
...
func (sdxx *TomoX) GetTriegc() *prque.Prque {
	return sdxx.Triegc
}

func (sdxx *TomoX) GetTradingStateRoot(block *types.Block, author common.Address) (common.Hash, error) {
...
func (sdxx *TomoX) UpdateOrderCache(baseToken, quoteToken common.Address, orderHash common.Hash, txhash common.Hash, lastState tradingstate.OrderHistoryItem) {
...
func (sdxx *TomoX) RollbackReorgTxMatch(txhash common.Hash) error {
	db := sdxx.GetMongoDB()
	db.InitBulk()
