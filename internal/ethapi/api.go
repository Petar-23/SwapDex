...
func (s *PublicBlockChainAPI) GetMasternodes(ctx context.Context, b *types.Block) ([]common.Address, error) {
	...
	if b.Number().Int64() >= 0 {
		...
		if engine, ok := s.b.GetEngine().(*DPoS.DPoS); ok {
			// Get block epoc latest.
			lastCheckpointNumber := prevBlockNumber - (prevBlockNumber % s.b.ChainConfig().DPoS.Epoch)
			prevCheckpointBlock, _ := s.b.BlockByNumber(ctx, rpc.BlockNumber(lastCheckpointNumber))
			if prevCheckpointBlock != nil {
				masternodes = engine.GetMasternodesFromCheckpointHeader(prevCheckpointBlock.Header(), curBlockNumber, s.b.ChainConfig().DPoS.Epoch)
			}
		} else {
			log.Error("Undefined DPoS consensus engine")
		}
	}
...
func (s *PublicBlockChainAPI) GetCandidateStatus(ctx context.Context, coinbaseAddress common.Address, epochNumber rpc.EpochNumber) (string, error) {
	...
	for _, address := range candidateAddresses {
		...
		if address.String() != "0x0000000000000000000000000000000000000000" {
			candidates = append(candidates, DPoS.Masternode{Address: address, Stake: v})
		}
	}
...
func (s *PublicBlockChainAPI) rpcOutputBlockSigners(b *types.Block, ctx context.Context, masternodes []common.Address) ([]common.Address, error) {
	...
	if b.Number().Int64() > 0 {
		...
		if engine, ok := s.b.GetEngine().(*DPoS.DPoS); ok {
			// Get block epoc latest.
			lastCheckpointNumber := signedBlockNumber - (signedBlockNumber % s.b.ChainConfig().DPoS.Epoch)
			prevCheckpointBlock, _ := s.b.BlockByNumber(ctx, rpc.BlockNumber(lastCheckpointNumber))
			if prevCheckpointBlock != nil {
				masternodes := engine.GetMasternodesFromCheckpointHeader(prevCheckpointBlock.Header(), blockNumber, s.b.ChainConfig().DPoS.Epoch)
...
func GetSignersFromBlocks(b Backend, blockNumber uint64, blockHash common.Hash, masternodes []common.Address) ([]common.Address, error) {
	...
	if engine, ok := b.GetEngine().(*DPoS.DPoS); ok {
		limitNumber := blockNumber - blockNumber%b.ChainConfig().DPoS.Epoch + 2*b.ChainConfig().DPoS.Epoch - 1
