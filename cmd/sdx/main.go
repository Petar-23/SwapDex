...
var (
	...
	app = utils.NewApp(gitCommit, "the sdxchain command line interface")
	...
	nodeFlags = []cli.Flag{
		...
		utils.SdxXEnabledFlag,
		utils.SdxXDataDirFlag,
		utils.SdxXDBEngineFlag,
		utils.SdxXDBConnectionUrlFlag,
		utils.SdxXDBReplicaSetNameFlag,
		utils.SdxXDBNameFlag,
...
func init() {
	...
	app.Copyright = "Copyright (c) 2021 Sdxchain"
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, cfg sdxConfig) {
	...
	if _, ok := ethereum.Engine().(*posv.Posv); ok {
		go func() {
			started := false
			ok := false
			slaveMode := ctx.GlobalIsSet(utils.SdxSlaveModeFlag.Name)
			var err error
			if common.IsTestnet {
