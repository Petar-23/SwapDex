...
func loadConfig(file string, cfg *SDXConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, SDXConfig) {
	// Load defaults.
	cfg := SDXConfig{
	...
	if ctx.GlobalBool(utils.SDXTestnetFlag.Name) {
...
func makeFullNode(ctx *cli.Context) (*node.Node, SDXConfig) {
