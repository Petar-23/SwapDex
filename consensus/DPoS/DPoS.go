...
// Package DPoS implements the proof-of-stake-voting consensus engine.
package DPoS
...
// DPoS proof-of-stake-voting protocol constants.
var (
...
// DPoS is the proof-of-stake-voting consensus engine proposed to support the
...
// New creates a DPoS proof-of-stake-voting consensus engine with the initial
...
func (c *DPoS) VerifyHeader(chain consensus.ChainReader, header *types.Header, fullVerify bool) error {
...
func (c *DPoS) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, fullVerifies []bool) (chan<- struct{}, <-chan error) {
...
func (c *DPoS) verifyHeaderWithCache(chain consensus.ChainReader, header *types.Header, parents []*types.Header, fullVerify bool) error {
...
func (c *DPoS) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header, fullVerify bool) error {
	if common.IsTestnet {
		fullVerify = false
	}
...
func (c *DPoS) verifyCascadingFields(chain consensus.ChainReader, header *types.Header, parents []*types.Header, fullVerify bool) error {
...
func (c *DPoS) YourTurn(chain consensus.ChainReader, parent *types.Header, signer common.Address) (int, int, int, bool, error) {
...
func (c *DPoS) verifySeal(chain consensus.ChainReader, header *types.Header, parents []*types.Header, fullVerify bool) error {
...
func (c *DPoS) calcDifficulty(chain consensus.ChainReader, parent *types.Header, signer common.Address) *big.Int {
...
func (c *DPoS) CacheData(header *types.Header, txs []*types.Transaction, receipts []*types.Receipt) []*types.Transaction {
...
func (c *DPoS) CacheSigner(hash common.Hash, txs []*types.Transaction) []*types.Transaction {
	signTxs := []*types.Transaction{}
	for _, tx := range txs {
		if tx.IsSigningTransaction() {
			signTxs = append(signTxs, tx)
...
func (c *DPoS) GetDb() ethdb.Database {
	return c.db
}
...
func getM1M2(masternodes []common.Address, validators []int64, currentHeader *types.Header, config *params.ChainConfig) (map[common.Address]common.Address, uint64, error) {
	...
	if maxMNs > 0 {
		...
		if isForked {
			moveM2 = ((currentHeader.Number.Uint64() % config.DPoS.Epoch) / uint64(maxMNs)) % uint64(maxMNs)
		}
