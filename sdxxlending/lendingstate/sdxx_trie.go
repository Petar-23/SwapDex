...
// SdxXTrie wraps a trie with key hashing. In a secure trie, all
// access operations hash the key using keccak256. This prevents
// calling code from creating long chains of nodes that
// increase the access time.
//
// Contrary to a regular trie, a SdxXTrie can only be created with
...
type SdxXTrie struct {
	...
	secKeyCacheOwner *SdxXTrie // Pointer to self, replace the key cache on mismatch
}

// NewSdxXTrie creates a trie with an existing root node from a backing database
...
func NewSdxXTrie(root common.Hash, db *trie.Database) (*SdxXTrie, error) {
	if db == nil {
		panic("trie.NewSdxXTrie called without a database")
	}
	trie, err := trie.New(root, db)
	if err != nil {
		return nil, err
	}
	return &SdxXTrie{trie: *trie}, nil
}
...
//func (t *SdxXTrie) hashKey(key []byte) []byte {
//	h := newHasher(0, 0, nil)
//	h.sha.Reset()
//	h.sha.Write(key)
...
// the actual cache).
func (t *SdxXTrie) getSecKeyCache() map[string][]byte {
