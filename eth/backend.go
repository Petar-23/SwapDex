...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	if eth.chainConfig.DPoS != nil {
		c := eth.engine.(*DPoS.DPoS)
		...
		appendM2HeaderHook := func(block *types.Block) (*types.Block, bool, error) {
			...
			if m2 == eb {
				...
				header := block.Header()
				sighash, err := wallet.SignHash(accounts.Account{Address: eb}, DPoS.SigHash(header).Bytes())
		...
		c.HookPenalty = func(chain consensus.ChainReader, blockNumberEpoc uint64) ([]common.Address, error) {
			...
			prevEpoc := blockNumberEpoc - chain.Config().DPoS.Epoch
			if prevEpoc >= 0 {
				start := time.Now()
		...
		c.HookPenaltyTIPSigning = func(chain consensus.ChainReader, header *types.Header, candidates []common.Address) ([]common.Address, error) {
			prevEpoc := header.Number.Uint64() - chain.Config().DPoS.Epoch
			combackEpoch := uint64(0)
			comebackLength := (common.LimitPenaltyEpoch + 1) * chain.Config().DPoS.Epoch
			...
			if prevEpoc >= 0 {
				...
				listBlockHash := make([]common.Hash, chain.Config().DPoS.Epoch)
				...
				for i := uint64(1); i < chain.Config().DPoS.Epoch; i++ {
		...
		c.HookReward = func(chain consensus.ChainReader, stateBlock *state.StateDB, header *types.Header) (error, map[string]interface{}) {
			...
			foundationWalletAddr := chain.Config().DPoS.FoudationWalletAddr
			...
			if number > 0 && number-rCheckpoint > 0 && foundationWalletAddr != (common.Address{}) {
				...
				chainReward := new(big.Int).Mul(new(big.Int).SetUint64(chain.Config().DPoS.Reward), new(big.Int).SetUint64(params.Ether))
		...
		c.HookVerifyMNs = func(header *types.Header, signers []common.Address) error {
			...
			if number > 0 && number%common.EpocBlockRandomize == 0 {
				...
				if !bytes.Equal(header.Validators, validators) {
					return DPoS.ErrInvalidCheckpointValidators
				}
			}
			return nil
		}
...
func (s *Ethereum) ValidateMasternode() (bool, error) {
	...
	if s.chainConfig.DPoS != nil {
		...
		return false, fmt.Errorf("Only verify masternode permission in DPoS protocol")
	}
	return true, nil
}
...
func GetValidators(bc *core.BlockChain, masternodes []common.Address) ([]byte, error) {
	if bc.Config().DPoS == nil {
		return nil, core.ErrNotDPoS
	}
